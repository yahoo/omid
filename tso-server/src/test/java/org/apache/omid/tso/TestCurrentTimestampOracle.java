...
public class TestCurrentTimestampOracle {

    private static final Logger LOG = LoggerFactory.getLogger(TestCurrentTimestampOracle.class);
    ...
    @InjectMocks
    private CurrentTimestampOracleImpl currentTimestampOracle;
    ...
    public void testMonotonicTimestampGrowth() throws Exception {
        ...
        currentTimestampOracle.initialize();

        long last = currentTimestampOracle.next();
        for (int i = 0; i < (3 * TimestampOracleImpl.TIMESTAMP_BATCH); i++) {
            long current = currentTimestampOracle.next();
            assertTrue(current > last , "Not monotonic growth");
            last = current;
        }
        assertTrue(currentTimestampOracle.getLast() == last);
